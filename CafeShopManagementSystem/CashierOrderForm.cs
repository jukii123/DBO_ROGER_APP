...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        public CashierOrderForm()
        {
            ...
            OrderBy.Items.Add("ID");
            OrderBy.Items.Add("Product Name");
            OrderBy.Items.Add("Price");
        ...
        {
            // Cambia a CashierOrderFormProdData
            ...
            // Determinar el criterio de ordenación
            if (OrderBy.SelectedItem == null)
            {
                // Manejo de error si no hay selección en el combo box
                MessageBox.Show("Por favor, selecciona un criterio de ordenación.");
                return;
            }

            string selectedOrderBy = OrderBy.SelectedItem.ToString();

            switch (selectedOrderBy)
            {
                case "ID":
                    QuickSort(listData, product => product.ID, 0, listData.Count - 1);
                    break;
                case "Product Name":
                    QuickSort(listData, product => product.ProductName, 0, listData.Count - 1);
                    break;
                case "Price":
                    QuickSort(listData, product => Convert.ToSingle(product.Price), 0, listData.Count - 1);
                    break;
                // Agrega más casos aquí si es necesario
                default:
                    MessageBox.Show("Criterio de ordenación no válido.");
                    return;
            }
        ...
        public void QuickSort<T>(List<T> list, Func<T, IComparable> keySelector, int left, int right)
{
    int i = left, j = right;
    var pivot = keySelector(list[(left + right) / 2]);

    while (i <= j)
    {
        while (keySelector(list[i]).CompareTo(pivot) < 0) i++;
        while (keySelector(list[j]).CompareTo(pivot) > 0) j--;
        if (i <= j)
        {
            var tmp = list[i];
            list[i] = list[j];
    ...
    // Recursión
    if (left < j)
        QuickSort(list, keySelector, left, j);
    if (i < right)
        QuickSort(list, keySelector, i, right);
}
        ...
        private void orderByComboBox_SelectedIndexChanged(object sender, EventArgs e)
{
    displayAvailableProds();
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Lisq8\OneDrive\Documentos\cafe.mdf;Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
        ...
        private void cashierOrderForm_amount_KeyDown(object sender, KeyEventArgs e)
{
    if (e.KeyCode == Keys.Enter)
    {
        try
        {
            ...
            {
                // Mostrar el cambio, si es 0 también se mostrará correctamente
                cashierOrderForm_change.Text = getChange.ToString("0.00");
            }
        }
        catch (FormatException)
        {
            MessageBox.Show("Por favor ingresa un monto válido.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            cashierOrderForm_amount.Text = "";
            cashierOrderForm_change.Text = "";
        }
        catch (Exception ex)
